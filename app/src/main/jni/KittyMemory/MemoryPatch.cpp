...
MemoryPatch::MemoryPatch(uintptr_t absolute_address,
  ...
  MemoryPatch MemoryPatch::absoluteWithHex(void *absolute_address, std::string hex) {
    MemoryPatch patch;
    ...
    // backup current content
    KittyMemory::memRead(&patch._orig_code[0], reinterpret_cast<const void *>(absolute_address), patch._size);
    return patch;
  }
